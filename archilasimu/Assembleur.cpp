...
modes["Idx"] = "Indexé";
modes["Rel"] = "Relatif";
modes["ImmExt"] = "Immédiat Etendu";
modes["DirExt"] = "Direct Etendu";
modes["IndExt"] = "Indirect Etendu";
modes["IdxExt"] = "Indexé Etendu";
modes["RelExt"] = "Relatif Etendu";
buildMnemonics();
buildMenuStrings();
...
Assembleur::buildMnemonics() {
    ...
    addManyMmnemonics("JUMPC(A==0)",230,most);
    addManyMmnemonics("JUMPC(B==0)",240,most);
    addManyMmnemonics("JUMPC(A>0)",250,most);
    addManyMmnemonics("JUMPC(B>0)",260,most);
    addManyMmnemonics("JUMPC(A:Pair)",270,most);
    addManyMmnemonics("JUMPC(B:Pair)",280,most);
...
Assembleur::addMnemonics(std::string name, int code){
    ...
    if(mOpDescForMenu.find(exploded[0]) != mOpDescForMenu.end()) {
        OpDesc newOp;
        mOpDescForMenu[exploded[0]] = newOp;
    }
    /*
...
*/
    if(exploded.size()>1) {
        mOpDescForMenu[exploded[0]].vAdressingModes.push_back(exploded[1]);
    }
...
Assembleur::drawOpcodeBuilderWindow(ImDrawList* dl, ImVec2 window_pos){
    ...
    if (mShowAssistant) {
        ...
        ImGui::PushItemWidth(100);
            ...
            char bufAdr[9]; bufAdr[0]=0;
            ImGui::InputText("##C5",bufAdr,9);
            ...
            const char* items0[] = { "Option 1", "Option 2", "Option 3" };
            ImGui::Combo("##C0", &selectedItem0, items0, IM_ARRAYSIZE(items0));

            ImGui::SameLine();  
            // ComboBox 1
            static int selectedItem1 = 0;
            const char* items1[] = { "Option 1", "Option 2", "Option 3" };
            ImGui::Combo("##C1", &selectedItem1, items1, IM_ARRAYSIZE(items1));
            ...
            ImGui::Combo("##C2", &selectedItem2, items2, IM_ARRAYSIZE(items2));
            ...
            ImGui::Combo("##C3", &selectedItem3, items3, IM_ARRAYSIZE(items3));
            
            ImGui::SameLine(); 

            char buf[9]; buf[0]=0;
            ImGui::InputText("##C4",buf,9);

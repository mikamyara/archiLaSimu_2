...
function get_full_url_into(buffer,maxLen) { var url = window.location.href; stringToUTF8(url, buffer, maxLen); }
function get_base_url_into(buffer,maxLen) { var loc = window.location; var base = loc.origin + loc.pathname.replace(/[^\/]*$/, ""); stringToUTF8(base, buffer, maxLen); }
function set_canvas_size(width,height) { var canvas = document.getElementById("canvas"); canvas.style.width = width + "px"; canvas.style.height = height + "px"; }
...
var _optionsDownloadSucceeded = Module['_optionsDownloadSucceeded'] = createExportWrapper('optionsDownloadSucceeded', 1);
var _optionsDownloadFailed = Module['_optionsDownloadFailed'] = createExportWrapper('optionsDownloadFailed', 1);

...
(function() {
 ...
 loadPackage({
  "files": [ {
   "filename": "/fonts/Cousine-Regular.ttf",
   "start": 0,
   "end": 43912
  }, {
   "filename": "/fonts/DroidSans.ttf",
   "start": 43912,
   "end": 233956
  }, {
   "filename": "/fonts/Karla-Regular.ttf",
   "start": 233956,
   "end": 250804
  }, {
   "filename": "/fonts/ProggyClean.ttf",
   "start": 250804,
   "end": 292012
  }, {
   "filename": "/fonts/ProggyTiny.ttf",
   "start": 292012,
   "end": 327668
  }, {
   ...
   "filename": "/fonts/binary_to_compressed_c.cpp",
   "start": 771424,
   "end": 785216
  }, {
   "filename": "/imgs/logoEEA.png",
   "start": 785216,
   "end": 809259
  }, {
   "filename": "/imgs/logos.png",
   "start": 809259,
   "end": 898816
  } ],
  "remote_package_size": 898816
 });
})();
...
function _emscripten_get_element_css_size(target, width, height) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(6, 1, target, width, height);
...
function _emscripten_set_fullscreenchange_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(7, 1, target, userData, useCapture, callbackfunc, targetThread);
...
function _emscripten_set_resize_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(8, 1, target, userData, useCapture, callbackfunc, targetThread);
...
function _emscripten_set_wheel_callback_on_thread(target, userData, useCapture, callbackfunc, targetThread) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(9, 1, target, userData, useCapture, callbackfunc, targetThread);
 target = findEventTarget(target);
 if (!target) return -4;
...
var _environ_get = function(__environ, environ_buf) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(10, 1, __environ, environ_buf);
 var bufSize = 0;
...
var _environ_sizes_get = function(penviron_count, penviron_buf_size) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(11, 1, penviron_count, penviron_buf_size);
...
function _fd_close(fd) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(12, 1, fd);
 try {
  var stream = SYSCALLS.getStreamFromFD(fd);
  FS.close(stream);
  return 0;
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(14, 1, fd, offset_low, offset_high, whence, newOffset);
...
function _emscripten_set_window_title(title) {
 if (ENVIRONMENT_IS_PTHREAD) return proxyToMainThread(16, 1, title);
...
var proxiedFunctionTable = [ _proc_exit, exitOnMainThread, ___syscall_fcntl64, ___syscall_ioctl, ___syscall_openat, _emscripten_get_device_pixel_ratio, _emscripten_get_element_css_size, _emscripten_set_fullscreenchange_callback_on_thread, _emscripten_set_resize_callback_on_thread, _emscripten_set_wheel_callback_on_thread, _environ_get, _environ_sizes_get, _fd_close, _fd_read, _fd_seek, _fd_write, _emscripten_set_window_title ];
...
var ___start_em_js = Module["___start_em_js"] = 249716;

var ___stop_em_js = Module["___stop_em_js"] = 251315;

...
/*
void
Node::drawWires(NVGcontext* ctx,NVGcolor inBusColor,float inThickness,Vector2i inAbsPos) {

    for(int k=0;k<mWires.size();k++) {
        if(mWires[k] != nullptr && mWires[k]->mTargetNode != nullptr) {
            Node* destNode = mWires[k]->mTargetNode;
            nvgStrokeWidth(ctx,inThickness);
            nvgStrokeColor(ctx,ArchiBusColor(inBusColor,mWires[k]->mStatus));   
            //nvgResetTransform(ctx);
            Vector2i P1,P2;
            P1 =  Vector2i(inAbsPos.x() + mLocalPos.x()          ,inAbsPos.y() + mLocalPos.y());
            P2 =  Vector2i(inAbsPos.x() + destNode->mLocalPos.x(),inAbsPos.y() + destNode->mLocalPos.y());
            nvgBeginPath(ctx);
